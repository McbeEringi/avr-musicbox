...
void core(const uint16_t *d){
	...
	dt=8000000/256 * 240/pgm_read_word_near(d++) / MIN_NOTE; // 最小音価 Hz*BPM/min
	...
	do{
		...
		out[3]=((_t[3]=(++_t[3]==t[3])?0:_t[3])<(t[3]>>1))?v[3]:0;// 出力波形生成 50%矩形波
		out[2]=((_t[2]=(++_t[2]==t[2])?0:_t[2])<(t[2]>>1))?v[2]:0;
		out[1]=((_t[1]=(++_t[1]==t[1])?0:_t[1])<(t[1]>>1))?v[1]:0;
		out[0]=((_t[0]=(++_t[0]==t[0])?0:_t[0])<(t[0]>>1))?v[0]:0;
